...
def signup():
    ...
    with open("C:\\Users\\joey_\\OneDrive\\Desktop\\StockSim\\user_pass.json", "r") as file:        #Gets data from the user_pass json file and stores to data
        data = json.load(file)
    if session.get("password") in data.keys():          #tries to validate signup information
        ...
        if session.get("password") == None:              #if there is no data in the user signup orm, set the output to nothing becuase if there is nothing it defaults to something unwanted
            ...
            with open("C:\\Users\\joey_\\OneDrive\\Desktop\\StockSim\\user_pass.json", "w") as file:         #if the password is available, save data into user_pass.json as follows: {password:{username:10000}}
...
def login():
    ...
    with open("C:\\Users\\joey_\\OneDrive\\Desktop\\StockSim\\user_pass.json", "r") as file:
...
def buy():
    ...
    if stock_name == None or amount_of_stock == None:
        ...
        with open("C:\\Users\\joey_\\OneDrive\\Desktop\\StockSim\\user_pass.json","w") as file:         #update user account value after valid transaction
            for i in data.keys():
                if i == session.get("password"):
        ...
        with open("C:\\Users\\joey_\\OneDrive\\Desktop\\StockSim\\data.json", "r") as file:
            data = json.load(file)

        with open("C:\\Users\\joey_\\OneDrive\\Desktop\\StockSim\\data.json", "w") as file:
